    ...
    >
      <nav className="grid grid-cols-3 gap-4 p-4 w-full justify-items-center h-full items-center">
        ...
        {page[4] === "teams" ? (
          <a
            href={`/${page[4]}`}
            className="font-la-liga text-2xl sm:text-3xl flex items-center justify-center text-mainblack hover:scale-105 transition"
          >
            <span className="uppercase">{pageId}</span>
          </a>
        ) : (
          <div className="font-laliga text-2xl sm:text-3xl flex items-center justify-center text-mainblack cursor-default">
            <span className="uppercase">{pageId}</span>
          </div>
        )}
